var SPOT_ASG_TYPES = ['bundle-builder', 'io', 'mem', 'pco', 'slave', 'unknown'];
...
function getGroupNames(entries, tallyNames) {
	...
	entries.forEach((entry) => {
		tallyNames.forEach((tallyName) => {
			Object.keys(entry[tallyName]).forEach((groupName) => {
...
function getSpotChartDatasets() {
	...
	SPOT_ASG_TYPES.forEach((asgType, slot) => {
		let launchTotal = 0;
...
function getTallySum(entries, tallyName, groupName) {
	let sum = 0;
...
function newCrossTable(entries, evictionsTallyName, groupHeader1, groupHeader2, launchesTallyName) {
	...
	getGroupNames(entries, [evictionsTallyName, launchesTallyName]).forEach((groupKey) => {
		let groupNames = groupKey.split('/');

		let evictionCount = getTallySum(entries, evictionsTallyName, groupKey);
		let launchCount = getTallySum(entries, launchesTallyName, groupKey);

		table.push([groupNames[0], groupNames[1], launchCount, evictionCount, getRate(evictionCount, launchCount)]);
	});
...
function newGroupTable(entries, evictionsTallyName, groupHeader, launchesTallyName) {
	...
	getGroupNames(entries, [evictionsTallyName, launchesTallyName]).forEach((groupName) => {
		let evictionCount = getTallySum(entries, evictionsTallyName, groupName);
		let launchCount = getTallySum(entries, launchesTallyName, groupName);
...
document.getElementById('spot-interruption-master-asg-type-search').addEventListener('keyup', () => searchTable('spot-interruption-master-asg-type-search', 'spot-interruption-master-asg-type-table'));
