...
public class MonitorRunner {
	...
	public Map<Monitor, MonitorResult> run(
		long cycleTimestamp, Collection<Monitor> monitors) {
			...
			for (Map.Entry<Monitor, Future<MonitorResult>> entry :
				...
				monitorResultsMap.put(
					...
					_resolveMonitorResult(
						cycleTimestamp, entry.getValue(), monitor,
						startTimestamp));
			}
		}
		finally {
	...
	private MonitorResult _newUnknownMonitorResult(
		long cycleTimestamp, String message) {

		return new MonitorResult(
			message, null, MonitorResult.Status.UNKNOWN, cycleTimestamp);
	}

	private MonitorResult _resolveMonitorResult(
		long cycleTimestamp, Future<MonitorResult> future, Monitor monitor,
		long startTimestamp) {
		...
		try {
			...
			return new MonitorResult(
				...
				monitorResult.getStatus(), cycleTimestamp);
		}
