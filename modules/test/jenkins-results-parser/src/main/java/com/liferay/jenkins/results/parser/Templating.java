	...
	public static void template() {
		...
		while (propertiesIterator.hasNext()) {
			...
			if (key.startsWith("global.property(")) {
				...
				for (String jobName : value.split(",")) {
					...
					if (jobContent.contains("Build Flow") ||
							...
							if (!triggerBuilderChildJobName.equals("@!child.job.names!@")) {
								triggerBuilderChildJobName =
									triggerBuilderChildJobName.replace(
										"@!job.variation.name!@",
											project.getProperty("job.variation.name(" +
												jobName + ")"));
						...
						childJobNamesMap.put(
							jobName, StringUtils.join(triggerBuilderChildJobNames, ','));
					}
					...
					if (currentParentJobName2 != null) {
						String currentParentJobShortName2 = project.getProperty(
							"job.short.name(" + currentParentJobName2 + ")");
						String currentParentJobVariationName2 = project.getProperty(
					...
					if ((currentParentJobName == null) &&
						...
						String currentParentJobShortName1 = project.getProperty(
							"job.short.name(" + currentParentJobName1 + ")");
						String currentParentJobVariationName1 = project.getProperty(
					...
					if (currentParentJobName != null) {
						...
						if (jobName.contains("[component.name]")) {
							...
							for (String componentName : componentNames.split(",")) {
					  		childJobNames.add(
									jobName.replace("[component.name]", "[" +
										componentName + "]"));
							}
						}
						else {
				...
				project.setProperty(
					"master.top.level.jobs.xml(" + masterHostname + ")", sb.toString());

				String masterPrimaryView = project.getProperty(
				...
				for (String parentJobName : parentJobNames) {
					...
					String childJobNames = project.getProperty(
						"child.job.names(" + parentJobName + ")");
				...
				project.setProperty(
					"master.list.views.xml(" + masterHostname + ")", sb.toString());
				...
				for (String parentJobName : parentJobNames) {
					...
					for (String childJobName : childJobNames.split(",")) {
				  	if (childJobName.equals(parentJobName)) {
						...
						String jobContentType = project.getProperty(
							"job.content.type(" + parentJobName + ")");

						if (jobContentType.equals("BuildTrigger")) {
							String shortParentJobName = project.getProperty(
						...
						if (customJobParameters != null) {
							...
							project.setProperty(
								"custom.job.parameters(" + parentJobName + ")", customJobParameters);
						}
						else {
					...
					project.setProperty(
						"build.flow.downstream.jobs.xml(" + parentJobName + ")", sb.toString());
					project.setProperty(
						"child.job.names(" + parentJobName + ")",
					...
					project.setProperty(
						"job.invocation.instantiate.list(" + parentJobName + ")", sb.toString());
					...
					project.setProperty(
						"job.invocation.make.list(" + parentJobName + ")", sb.toString());
				}
			}
				...
				project.setProperty(
					"master.axes.label.slaves.xml(" + masterHostname + ")", sb.toString());
			}
		...
		project.setProperty(
			"master.hostnames", StringUtils.join(masterHostnames, ","));
		project.setProperty(
			"slave.hostnames", StringUtils.join(slaveHostnames, ","));
