...
public class MonitorScheduler {
	...
	public List<Monitor> getDueMonitors(
		long cycleTimestamp, List<Monitor> monitors) {

		List<Monitor> dueMonitors = new ArrayList<>();

		for (Monitor monitor : monitors) {
			if (_isDue(cycleTimestamp, monitor)) {
	...
	private boolean _isDue(long cycleTimestamp, Monitor monitor) {
		...
		if ((cycleTimestamp - latestMonitorResult.getTimestamp()) >=
