...
public class ResourceThresholdMonitor extends BaseMonitor {
		...
		if ((_criticalThreshold <= 0) && (_warnThreshold <= 0)) {
			throw new IllegalArgumentException(
				JenkinsResultsParserUtil.combine(
					"Missing required property ", _getThresholdKey("warn"),
					" or ", _getThresholdKey("critical")));
		}
	}
	...
	private String _getIndeterminateMessage() {
		if (_selector == null) {
			return JenkinsResultsParserUtil.combine(
				"Unable to determine ", _metric, " for ", _masterName);
		}

		return JenkinsResultsParserUtil.combine(
			"Unable to determine ", _metric, " for ", _selector, " on ",
			_masterName);
	}
